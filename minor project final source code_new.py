...
# # minor final project source code
...
def createHogDescriptor(image):
    H = hog(image, orientations=8, pixels_per_cell=(16, 16),cells_per_block=(2, 2),transform_sqrt=True,visualise=False,multichannel=True)
    return H
...
def extract_images(mask,real,count,height,width):
    mid = real.shape[0]-(real.shape[0]//3)
    add=50
    ...
    for cnt in contours:
        ...
        cx=int(m['m10']/m['m00'])
        cy=int(m['m01']/m['m00'])
        if h>w and cy>=(height-height//3) and cy<=(height-height//3+add) and cv2.contourArea(cnt)>500:
            ...
            if ans==1:  #If object is bike
                cv2.circle(real,(cx,cy),5,(0,0,255),-1)    #blue dot
                ...
                if ans==0:  #iIf object has helmet
                    cv2.circle(real,(cx,cy),5,(0,255,0),-1) #green dot
                cv2.rectangle(real,(x,y),(x+w,y+h),(0,0,255),1)   
            else:
                cv2.circle(real,(cx,cy),10,(255,0,0),-1)    #red dot
...
class HomePage(Frame):
    def get_frame(self):
        ...
        self.cap=cv2.VideoCapture('first.mp4')
        ...
        while True:
            ...
            if ret:
               ...
               # mask_frame=fgbg.apply(first_frame)
                ...
                frame=extract_images(mask_frame,original_frame,count,height,width)
    ...
    def __init__(self,parent,controller):
        ...
        self.lbl=Label(self.container,text='|Live Streaming|',bg='powder blue',font=('arial',20,'bold'))
        ...
        #self.left = Frame(container,highlightbackground="green",bg="green", highlightcolor="green", highlightthickness=2, borderwidth=2,bd=0, relief="solid")
        #self.ll=Label(self.left, text="Live streaming")
        #self.left.pack(side="left", expand=True, fill="both")
        #self.right = Frame(container,highlightbackground="red",bg="green", highlightcolor="red", highlightthickness=2, borderwidth=2,bd=0, relief="solid")
        #self.right.pack(side="right", expand=True, fill="both")
        #self.ll.grid(row=0,column=1,sticky='news')
...
class DataViewPage(Frame):
    def __init__(self,parent,controller):
        ...
        df=pd.read_csv('second.txt')
        df.columns=["X-axis","Y-axis"]
        df=df.sort_values(by="X-axis")
...
class GraphPage(Frame):
    def __init__(self,parent,controller):
        ...
        a = f.add_subplot(111)
        pulldata=open("second.txt","r").read()
...
class AboutPage(Frame):
    def __init__(self,parent,controller):
        ...
        label=Label(self,text=text,font=('arial',10,'bold'))
...
class Login(Tk):
    ...
    def send(self,u,p):
        sender='sachinkushwaha142@gmail.com'
        receiver=u
        password='9200346400'
